...
function tmaddE(){
    ...
    function (e){
        ...
        if( tg == "n" ){
            //clickでコンテキストメニュー消すため
            //clickだと最後に実行されるためtg変更されてないtouchendまで実行されない気がする
            $( "body" )[0].addEventListener( "click", deleteTmctxm, false ); 
    ...
    //名前変更
    ctxm1.addEventListener( "touchstart",
    function(){
    ...
    //削除
    ctxm2.addEventListener( "touchstart",
    function(){
...
//長押し判定(contextmenuが長押しに対応していたため使ってない)
function nagaosi( e ){
    console.log( "nagaosi" );
    if( tg != "n" ){
        console.log( "tmcvs ctxmenu {" );
        console.log( "contextmenu" );
        ctxm.style.left=clix+"px";
        ctxm.style.top=cliy+"px";
        ctxm.style.display="block";
        tg = "c";
        console.log( "tg", tg, "tp", tp );
        console.log( "}" );
...
function drowts(){
    ...
    console.log( "tg", tg );
    if( tg == "n" ){
        clix = event.changedTouches[0].clientX;
        cliy = event.changedTouches[0].clientY;
        console.log( "clix", clix, "cliy", cliy, "scroll", $( "#tm" )[0].scrollLeft );
        tp = settp( clix+$( "#tm" )[0].scrollLeft, cliy-wx*2 );
        console.log( "tp", tp );
        ...
        if( tp < 192 ){
            ...
            if( dai[ tp ] == "" ){
                ...
                let a = clix+$( "#tm" )[0].scrollLeft - tp%96*wx-wh;
                ...
                setint = setInterval( nagaosi, 1000 );
                intc = "been";
            }
...
function drowtm(e){
    ...
    if( tg == "a" || tg == "er" || tg == "el" ){
        ...
        tp = settp( event.changedTouches[0].clientX+$( "#tm" )[0].scrollLeft, event.changedTouches[0].clientY-wy*2 );
        console.log( "tp", tp );
        if( tp > 191 ){
            tp = 191;
        }
...
function deleteTmctxm(){
    console.log( "hide tm" );
    ctxm.style.display="none";
    $( "body" )[0].removeEventListener( "click", deleteTmctxm, false );
}
